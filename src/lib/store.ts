...
interface State {
  ...
  isAdmin: boolean;
  setFilter: <
    K extends keyof Pick<State, "mood" | "companion" | "budget" | "duration" | "currency">,
  >(
    k: K,
    v: State[K],
  ) => void;
...
// Bridge so the auth/profile layer can increment the user's itinerary counter
// in the database whenever a plan is generated, without coupling the store to auth.
let bumpItineraries: () => void = () => {};
export function registerItineraryBump(fn: () => void) {
  bumpItineraries = fn;
}
...
export const useStore = create<State>()(
  persist(
    (set, get) => ({
      ...
      plan: [],
      isAdmin: false,
      setFilter: (k, v) => set({ [k]: v } as Partial<State>),
      ...
      generatePlan: () => {
        ...
        set({ plan });
        if (plan.length) bumpItineraries();
      },
      surpriseMe: () => {
        const { duration } = get();
