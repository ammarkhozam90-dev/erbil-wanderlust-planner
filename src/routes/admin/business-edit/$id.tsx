        ...
        return (
          <button key={o} type="button" onClick={() => toggle(o)}>
            <Badge
              ...
              className={cn(
                "cursor-pointer px-3 py-1.5 text-sm capitalize",
                active && "bg-primary",
              )}
            >
              {o}
...
function EditBusiness() {
  ...
  const photosQ = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("merchant_photos").select("*").eq("merchant_id", id).order("sort_order"))
        .data ?? [],
  });
  ...
  const hoursQ = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("merchant_hours").select("*").eq("merchant_id", id).order("day_of_week"))
        .data ?? [],
  });
  ...
  useEffect(() => {
    if (businessQ.data && !form) {
      setForm({
        ...businessQ.data,
        categories: businessQ.data.categories?.length
          ? businessQ.data.categories
          : [businessQ.data.category],
      });
    }
  }, [businessQ.data, form]);
  ...
  async function saveHours() {
    ...
    const { error } = await supabase
      .from("merchant_hours")
      .upsert(payload, { onConflict: "merchant_id,day_of_week" });
  ...
  async function uploadPhoto(kind: "logo" | "cover" | "gallery", file: File) {
    setUploading(kind);
    const previousUrl = kind === "logo" ? form.logo_url : kind === "cover" ? form.cover_url : null;
    let toUpload: Blob;
    try {
    ...
    if (kind === "gallery") {
      const { error: insertErr } = await supabase
        ...
        .insert({ merchant_id: id, url: data.publicUrl, sort_order: photosQ.data?.length ?? 0 });
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Features &amp; dietary options</CardTitle>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            <Label className="mb-2 block">Best visit time</Label>
            <MultiPick
              options={TIMES}
              value={form.best_visit_time ?? []}
              onChange={(v) => update("best_visit_time", v)}
            />
          </div>
          <div>
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-2">
              <Label>Logo</Label>
              {form.logo_url && (
                <img src={form.logo_url} className="h-20 w-20 rounded object-cover" alt="logo" />
              )}
          ...
          <div className="space-y-2">
            ...
            <UploadButton
              ...
              onPick={(f) => uploadPhoto("gallery", f)}
            />
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {hourRows.map((row, i) => (
            <div
              key={i}
              className="grid grid-cols-1 items-center gap-3 rounded border p-3 md:grid-cols-[110px_1fr_1fr_auto_auto]"
            >
              ...
              <label className="flex items-center gap-2 text-sm">
                <Switch
                  checked={row.is_24h}
                  onCheckedChange={(v) => updateHour(i, { is_24h: v, is_closed: false })}
                />{" "}
                24h
              </label>
              <label className="flex items-center gap-2 text-sm">
                <Switch
                  ...
                  onCheckedChange={(v) => updateHour(i, { is_closed: v, is_24h: false })}
                />{" "}
                Closed
              </label>
            </div>
