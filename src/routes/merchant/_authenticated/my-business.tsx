    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {options.map((o) => {
          ...
          return (
            <button key={o} type="button" onClick={() => toggle(o)} className="transition-transform active:scale-95">
              <Badge variant={active ? 'default' : 'outline'} className={cn('cursor-pointer px-3 py-1 text-xs capitalize transition-all', active ? 'bg-gold text-background hover:bg-gold/90 border-transparent' : 'hover:border-gold/50')}>
                {o}
              </Badge>
            </button>
          );
...
function MyBusiness() {
  ...
  const hoursQ = useQuery({
    ...
    enabled: !!m?.id,
    queryFn: async () => {
      const { data, error } = await supabase.from('merchant_hours').select('*').eq('merchant_id', m!.id).order('day_of_week');
  ...
  const photosQ = useQuery({
    ...
    enabled: !!m?.id,
    queryFn: async () => {
      const { data, error } = await supabase.from('merchant_photos').select('*').eq('merchant_id', m!.id).order('sort_order');
      if (error) throw error;
  ...
  if (isLoading || !m || !form) return <div className="flex min-h-screen items-center justify-center text-muted-foreground"><Sparkles className="mr-2 h-5 w-5 animate-pulse text-gold" /> Loading your workspace…</div>;
  ...
  async function uploadPhoto(kind: 'logo' | 'cover' | 'gallery', file: File) {
    ...
    if (kind === 'gallery') {
      ...
      if (previousUrl?.includes(marker)) {
        const oldPath = decodeURIComponent(previousUrl.slice(previousUrl.indexOf(marker) + marker.length));
    ...
    <div className="fixed inset-0 z-50 overflow-hidden bg-[#090b0b] p-2 md:p-5 lg:p-8">
      ...
      <header className="flex h-20 shrink-0 items-center justify-between border-b border-gold/10 bg-card/40 px-6 backdrop-blur-xl md:px-12">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate({ to: '/merchant/dashboard' })} className="grid h-10 w-10 place-items-center rounded-full border border-border bg-background/50 transition-colors hover:bg-accent">
            <X className="h-5 w-5" />
          </button>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-gold">Setup Wizard</span>
              <div className="h-1 w-1 rounded-full bg-gold/30" />
              <span className="text-[10px] font-medium text-muted-foreground">Step {activeStep + 1} of {STEPS.length}</span>
            </div>
            <h2 className="font-display text-lg font-bold tracking-tight">{STEPS[activeStep].label}</h2>
          </div>
        </div>

        <div className="hidden items-center gap-2 md:flex">
          {STEPS.map((s, i) => (
            <button key={s.id} onClick={() => i <= activeStep && setActiveStep(i)} className={cn('h-1.5 rounded-full transition-all', i === activeStep ? 'w-8 bg-gold' : i < activeStep ? 'w-4 bg-gold/40' : 'w-4 bg-border hover:bg-gold/20')} title={s.label} />
          ))}
        </div>

        <Button variant="ghost" size="sm" onClick={() => saveAll()} disabled={saving} className="text-xs font-semibold uppercase tracking-wider text-gold hover:bg-gold/10">
          {saving ? 'Saving…' : <><Save className="mr-2 h-3.5 w-3.5" /> Save Draft</>}
        </Button>
      </header>
        ...
        <div className="mx-auto max-w-3xl animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="space-y-8 pb-32">
            {activeStep === 0 && (
              <div className="grid gap-8">
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Business Name</Label><Input value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="e.g. Citadel View Restaurant" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /><FieldError show={attemptedSubmit && !form.name?.trim()} message="Name is required" /></div>
                  <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Primary Phone</Label><Input value={form.phone} onChange={(e) => update('phone', e.target.value)} placeholder="+964 …" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /><FieldError show={attemptedSubmit && !form.phone?.trim()} message="Phone is required" /></div>
                </div>
                <div className="space-y-3"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Category</Label><div className="flex flex-wrap gap-2">{CATEGORIES.map((c) => { const active = (form.categories ?? []).includes(c); return (<button key={c} type="button" onClick={() => toggleCategory(c)} className="transition-transform active:scale-95"><Badge variant={active ? 'default' : 'outline'} className={cn('cursor-pointer px-4 py-2 text-xs capitalize transition-all', active ? 'bg-gold text-background border-transparent' : 'hover:border-gold/50')}>{c}</Badge></button>); })}</div><FieldError show={attemptedSubmit && !form.categories?.length} message="Select at least one category" /></div>
                <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Description</Label><Textarea value={form.description} onChange={(e) => update('description', e.target.value)} placeholder="Tell travelers what makes your place special…" rows={5} className="border-gold/10 bg-card focus-visible:ring-gold" /></div>
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Public Email</Label><Input value={form.email} onChange={(e) => update('email', e.target.value)} placeholder="hello@business.com" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /></div>
                  <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Website</Label><Input value={form.website} onChange={(e) => update('website', e.target.value)} placeholder="https://…" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /></div>
                </div>
              </div>
            )}
            ...
            {activeStep === 1 && (
              <div className="grid gap-8">
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Street Address</Label><Input value={form.address} onChange={(e) => update('address', e.target.value)} placeholder="e.g. 100m Road, near Citadel" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /></div>
                  <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">City / District</Label><Input value={form.city} onChange={(e) => update('city', e.target.value)} placeholder="e.g. Ankawa, Erbil" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /></div>
                </div>
                <div className="relative h-[450px] w-full overflow-hidden rounded-3xl border border-gold/20 shadow-xl">
                  <MapPicker lat={form.latitude} lng={form.longitude} onChange={(lat, lng) => { update('latitude', lat); update('longitude', lng); }} />
                </div>
                <FieldError show={attemptedSubmit && (form.latitude == null || form.longitude == null)} message="Please pin your location on the map" />
              </div>
            )}

            {activeStep === 2 && (
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Instagram</Label><Input value={form.instagram} onChange={(e) => update('instagram', e.target.value)} placeholder="@username" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /></div>
                <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Facebook</Label><Input value={form.facebook} onChange={(e) => update('facebook', e.target.value)} placeholder="URL" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /></div>
                <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Tiktok</Label><Input value={form.tiktok} onChange={(e) => update('tiktok', e.target.value)} placeholder="@username" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /></div>
                <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Whatsapp</Label><Input value={form.whatsapp} onChange={(e) => update('whatsapp', e.target.value)} placeholder="+964 …" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /></div>
              </div>
            )}
                ...
                <div className="grid gap-10 md:grid-cols-2">
                  <div className="space-y-4">
                    ...
                    <div className="group relative aspect-square w-40 overflow-hidden rounded-3xl border-2 border-dashed border-gold/20 bg-card transition-all hover:border-gold/50">
                      {m?.logo_url ? <img src={m.logo_url} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center"><ImageOff className="h-8 w-8 text-muted-foreground/30" /></div>}
                      <label className="absolute inset-0 flex cursor-pointer items-center justify-center bg-background/60 opacity-0 transition-opacity group-hover:opacity-100"><Upload className="mr-2 h-5 w-5 text-gold" /><span className="text-xs font-bold text-gold">Upload</span><input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && pickForCrop('logo', e.target.files[0])} /></label>
                    </div>
                    <FieldError show={attemptedSubmit && !m?.logo_url} message="A logo helps travelers recognize you" />
                  </div>
                  <div className="space-y-4">
                    ...
                    <div className="group relative aspect-video w-full overflow-hidden rounded-3xl border-2 border-dashed border-gold/20 bg-card transition-all hover:border-gold/50">
                      {m?.cover_url ? <img src={m.cover_url} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center"><ImageOff className="h-8 w-8 text-muted-foreground/30" /></div>}
                      <label className="absolute inset-0 flex cursor-pointer items-center justify-center bg-background/60 opacity-0 transition-opacity group-hover:opacity-100"><Upload className="mr-2 h-5 w-5 text-gold" /><span className="text-xs font-bold text-gold">Upload Cover</span><input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && pickForCrop('cover', e.target.files[0])} /></label>
                    </div>
                    <FieldError show={attemptedSubmit && !m?.cover_url} message="High quality cover images increase clicks" />
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
                    {photosQ.data?.map((p) => (
                      <div key={p.id} className="group relative aspect-square overflow-hidden rounded-2xl border border-gold/10 bg-card">
                        <img src={p.url} alt="" className="h-full w-full object-cover" onError={() => setBrokenPhotoIds((s) => new Set(s).add(p.id))} />
                        <button onClick={async () => { await supabase.from('merchant_photos').delete().eq('id', p.id); qc.invalidateQueries({ queryKey: ['merchant-photos', m!.id] }); }} className="absolute right-2 top-2 rounded-lg bg-destructive/90 p-1.5 text-white opacity-0 transition-opacity group-hover:opacity-100"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    ))}
                    <label className="flex aspect-square cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-gold/10 bg-card/50 transition-all hover:border-gold/40 hover:bg-gold/5"><Plus className="h-6 w-6 text-gold" /><span className="text-[10px] font-bold uppercase tracking-wider text-gold">Add Photo</span><input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && uploadPhoto('gallery', e.target.files[0])} /></label>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4 rounded-3xl border border-gold/10 bg-card/30 p-6">
                {hourRows.map((h, i) => (
                  <div key={h.day_of_week} className="flex flex-wrap items-center gap-4 border-b border-border/40 pb-4 last:border-0 last:pb-0">
                    <div className="w-24 text-sm font-bold uppercase tracking-wider text-muted-foreground">{DAYS[h.day_of_week]}</div>
                    <div className="flex flex-1 items-center gap-6">
                      <div className="flex items-center gap-2"><Input type="time" className="w-32 border-gold/10 bg-background" disabled={h.is_closed || h.is_24h} value={h.open_time || '09:00'} onChange={(e) => updateHour(i, { open_time: e.target.value })} /><span className="text-xs font-medium text-muted-foreground">to</span><Input type="time" className="w-32 border-gold/10 bg-background" disabled={h.is_closed || h.is_24h} value={h.close_time || '22:00'} onChange={(e) => updateHour(i, { close_time: e.target.value })} /></div>
                      <div className="flex items-center gap-4">
                        <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-muted-foreground"><Switch checked={h.is_24h} onCheckedChange={(v) => updateHour(i, { is_24h: v, is_closed: false })} className="data-[state=checked]:bg-gold" /> 24h</label>
                        <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-muted-foreground"><Switch checked={h.is_closed} onCheckedChange={(v) => updateHour(i, { is_closed: v, is_24h: false })} className="data-[state=checked]:bg-destructive" /> Closed</label>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {activeStep === 6 && (
              <div className="grid gap-10">
                ...
                <div className="grid gap-8 md:grid-cols-2">
                  <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Best time to visit</Label><Select value={form.best_visit_time} onValueChange={(v) => update('best_visit_time', v)}><SelectTrigger className="h-12 border-gold/10 bg-card"><SelectValue placeholder="Select time…" /></SelectTrigger><SelectContent>{TIMES.map((t) => <SelectItem key={t} value={t} className="capitalize">{t}</SelectItem>)}</SelectContent></Select></div>
                  <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Price Level</Label><Select value={form.price_level} onValueChange={(v) => update('price_level', v)}><SelectTrigger className="h-12 border-gold/10 bg-card"><SelectValue placeholder="Select level…" /></SelectTrigger><SelectContent>{PRICES.map((p) => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select></div>
                </div>
                ...
                <div className="space-y-2"><Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Average visit duration (minutes)</Label><Input type="number" value={form.avg_duration_minutes} onChange={(e) => update('avg_duration_minutes', e.target.value)} placeholder="e.g. 60" className="h-12 border-gold/10 bg-card focus-visible:ring-gold" /></div>
              </div>
            )}
            ...
            {activeStep === 8 && (
              <div className="space-y-8 text-center">
                ...
                <div className="mx-auto max-w-md rounded-2xl border border-gold/10 bg-card/40 p-6 text-left">
                  <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-gold">Final Checklist</h3>
                  <div className="space-y-3">
                    {checks.map(c => (
                      <div key={c.id} className="flex items-center justify-between gap-3 text-sm">
                        <span className="text-muted-foreground">{c.label}</span>
                        {c.ok ? <Badge variant="secondary" className="bg-emerald-500/10 text-emerald-500">Ready</Badge> : <button onClick={() => setActiveStep(c.step)} className="text-[10px] font-bold uppercase text-gold hover:underline">Missing</button>}
                      </div>
                    ))}
                  </div>
                </div>
                <Button onClick={handleSubmitForReview} disabled={submitting || m.status === 'pending'} className="h-14 w-full max-w-md bg-gold text-lg font-bold text-background shadow-luxury hover:bg-gold/90">{submitting ? 'Submitting…' : m.status === 'pending' ? 'Already Pending' : <><Send className="mr-2 h-5 w-5" /> Submit for Review</>}</Button>
              </div>
            )}
          </div>
        </div>
      ...
      <footer className="z-20 flex shrink-0 items-center justify-between gap-3 border-t border-gold/15 bg-card/75 px-3 py-3 backdrop-blur-2xl md:gap-6 md:px-6 md:py-4">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="mt-1 flex gap-1.5">
            {STEPS.map((_, i) => (
              <div key={i} className={cn('h-1 w-1 rounded-full transition-all', i === activeStep ? 'w-4 bg-gold' : i < activeStep ? 'bg-gold/40' : 'bg-border')} />
            ))}
          </div>
        </div>

        {activeStep < STEPS.length - 1 ? (
          <button onClick={goNext} className="flex h-12 items-center gap-2 rounded-full bg-gold px-8 text-sm font-bold text-background shadow-lg transition-all hover:bg-gold/90 active:scale-95">
      ...
      <ImageCropDialog
        ...
        onCancel={() => { if (cropTarget) URL.revokeObjectURL(cropTarget.src); setCropTarget(null); }}
        onConfirm={async (blob) => { if (!cropTarget) return; const { kind, src } = cropTarget; URL.revokeObjectURL(src); setCropTarget(null); const croppedFile = new File([blob], `${kind}.jpg`, { type: 'image/jpeg' }); await uploadPhoto(kind, croppedFile); }}
      />
    </div>
  );
}
