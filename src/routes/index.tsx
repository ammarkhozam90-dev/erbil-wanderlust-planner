...
const SIZE_PX: Record<string, string> = { sm: "20px", md: "30px", lg: "46px", xl: "60px" };
const SUB_SIZE_PX: Record<string, string> = { sm: "14px", md: "16px", lg: "18px", xl: "22px" };

// Fallback layout — used until the admin sets custom content in
// /admin/site-content, or if the site_hero row can't be reached.
const DEFAULT_LAYOUT = {
  headline: {
    x: 8, y: 34, maxWidth: 55,
    words: [
      { text: "EXPLORE", color: "#F5F0E6", font: "display", size: "lg" },
      { text: "ERBIL", color: "#F5F0E6", font: "display", size: "lg" },
      { text: "FROM", color: "#D4AF37", font: "display", size: "lg" },
      { text: "WITHIN", color: "#F5F0E6", font: "display", size: "lg" },
    ],
  },
  subheadline: {
    text: "AI-powered personalized plans for residents, tourists, families, couples and remote workers.",
    x: 8, y: 56, maxWidth: 42, color: "#E5DFD0", font: "sans", size: "sm",
  },
  buttons: [
    { label: "Generate My Plan", x: 8, y: 66, style: "primary" },
    { label: "Surprise Me", x: 28, y: 66, style: "secondary" },
  ],
};
      ...
      <div className="mx-auto w-full max-w-[1600px] px-4 py-6 lg:px-8">
        <div className="space-y-8">
          ...
          <section className="relative overflow-hidden rounded-3xl shadow-luxury">
            ...
            {/* Headline — positioned exactly as set in the admin editor */}
            <div
              className="absolute leading-[1.05]"
              style={{ left: `${layout.headline.x}%`, top: `${layout.headline.y}%`, maxWidth: `${layout.headline.maxWidth}%` }}
            >
              {layout.headline.words.map((w: any, i: number) => (
                <span
                  key={i}
                  className={`mr-2 inline-block font-bold ${w.font === "display" ? "font-display" : "font-sans"}`}
