...
function SettingsPage() {
  const { session, loading, changePassword, deleteAccount, signOut, signOutOtherSessions } = useAuth();
                ...
                <Dialog open={showSessions} onOpenChange={setShowSessions}>
                  ...
                  <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        For your security, we only show this device here. You can sign out of every other device at once.
                      </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-2">
                      <div className="flex items-center gap-3 rounded-xl border border-gold/40 bg-gold/5 p-3">
                        <div className="rounded-full bg-gold/10 p-2 text-gold"><ShieldCheck className="h-4 w-4" /></div>
                        <div className="flex-1">
                          <p className="text-xs font-semibold">This device</p>
                          <p className="text-[11px] text-muted-foreground">
                            {session?.user?.email} · Signed in {session?.user?.last_sign_in_at
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        If you signed in anywhere else — another browser, phone, or shared computer — the button below will sign all of them out. This device will stay signed in.
                      </p>
                    </div>
                    <DialogFooter>
                      <Button
                        ...
                        className="w-full"
                      >
                        {sessionsBusy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
